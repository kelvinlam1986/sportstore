...
namespace SpyStore.DAL.Initializers
{
	public static class StoreSampleData
	{
		...
		public static IEnumerable<Customer> GetAllCustomersRecords(StoreContext context) => new List<Customer>
		{
			new Customer { EmailAddress = "spy@secret.com", Password = "Foo", FullName = "Super Spy" }
		};
		...
		{
			var prod1 = context.Categories.Include(c => c.Products).FirstOrDefault()?.Products.FirstOrDefault();
			var prod2 = context.Categories.Skip(1).Include(c => c.Products).FirstOrDefault()?.Products.FirstOrDefault();
			var prod3 = context.Categories.Skip(2).Include(c => c.Products).FirstOrDefault()?.Products.FirstOrDefault();
		...
		public static List<OrderDetail> GetOrderDetails(StoreContext context)
		{
			var prod1 = context.Categories.Include(c => c.Products).FirstOrDefault()?.Products.FirstOrDefault();
			var prod2 = context.Categories.Skip(1).Include(c => c.Products).FirstOrDefault()?.Products.FirstOrDefault();
			var prod3 = context.Categories.Skip(2).Include(c => c.Products).FirstOrDefault()?.Products.FirstOrDefault();
